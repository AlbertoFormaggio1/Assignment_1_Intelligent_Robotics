...
void Utilities::keep_circles(std::unordered_map<int, std::vector<cv::Point2f>> &cluster_dict,
    ...
    for (const auto &entry: cluster_dict) {
        ...
        if (!lines.empty()) {
            ...
            first = cluster_points[cluster_points.size() / 4];
            mid = cluster_points[cluster_points.size() / 2];
            last = cluster_points[cluster_points.size() * 3 / 4];
            ...
            if (!Utilities::find_circle(first, mid, last, x, y, r)) {
                ...
                cv::Point2f c(x, y);
                if (!Utilities::is_circle(cluster_points, c, r)) {
...
bool Utilities::is_circle(std::vector<cv::Point2f> points, cv::Point2f center, float r) {
    ...
    for(int i = 1; i < points.size() - 1; i++){
        ...
        ROS_INFO("m1: %f; m2: %f, angle: %f", m1, m2, angle);
...
bool Utilities::find_circle(cv::Point2f first, cv::Point2f mid, cv::Point2f last, float &xc, float &yc, float &r) {
    ...
    ROS_INFO("xc: %f, yc: %f, r: %f..... first.x: %f, first.y: %f", xc, yc, r, first.x, first.y);
    return true;
}
