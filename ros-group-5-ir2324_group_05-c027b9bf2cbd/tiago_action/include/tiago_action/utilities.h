...
namespace Utilities{
    ...
    const float MAX_RADIUS = 0.3; // Maximum radius for an obstacle to be considered as a circle
    ...
    bool is_circle(std::vector<cv::Point2f> points, cv::Point2f center, float r);
    ...
    void keep_circles(std::unordered_map<int, std::vector<cv::Point2f>>& cluster_dict, std::vector<cv::Point2f>& centers);
